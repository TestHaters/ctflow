import { ButtonNodeCompiler } from './nodeCompilers/buttonNodeCompiler';
import { CheckboxNodeCompiler } from './nodeCompilers/CheckboxNodeCompiler';
import { ContainsNodeCompiler } from './nodeCompilers/ContainsNodeCompiler';
import { TextInputNodeCompiler } from './nodeCompilers/TextInputNodeCompiler';
import { VisitPageNodeCompiler } from './nodeCompilers/VisitPageNodeCompiler';
import { WaitNodeCompiler } from './nodeCompilers/WaitNodeCompiler';
...
export class Compiler {
  static compile(store: Store): string {
    ...
    // store = fakeStore
    let compiledText = '';
    const orderNodes = this.buildNodeChain(store);
  ...
  static findRootNode(store: Store) {
  ...
  static buildNodeChain(store: Store): any[] {
    let currentNode = this.findRootNode(store);
    let currentEdge: any;
    const orderedNodes = [currentNode];

    // prevent infinity loop
    for (
      let counter = 0;
      counter < Object.keys(store.edges).length;
      counter++
    ) {
